...
void registerOMPasses(int optLevel) {
  ...
  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createONNXToOneDNNPass();
  });

  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createMergeGPUModulesPass();
  });

  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createOptimizeGPUStreamPass();
  });

  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createInsertGpuDeallocPass();
  });
  ...
  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createLower4DMatMulPass();
  });

  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createSNNStructureAnnotationPass();
  });

  mlir::registerPass([]() -> std::unique_ptr<mlir::Pass> { //modified by p
    return createSNNBatchFusionPass();
  });
