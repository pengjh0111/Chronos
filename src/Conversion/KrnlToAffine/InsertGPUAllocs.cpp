  ...
  module.walk([&](func::FuncOp funcOp) {
    ...
    for (auto launchOp : launchOps) {
      ...
      for (Value operand : operands) {
        ...
        // 调用 processOperand 来处理各种情况，包括 reinterpret_cast
        Value processedMemref = processOperand(builder, launchOp.getLoc(), operand, updateUsers);
        if (processedMemref != operand) {
          newOperands.push_back(processedMemref);
          launchChanged = true;
        } else {
          newOperands.push_back(operand);
        }
