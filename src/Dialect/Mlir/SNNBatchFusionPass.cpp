...
private:
  ...
  //   LLVM_DEBUG(llvm::dbgs() << "Allocated GPU memory for batch matmul fusion\n");
  ...
  //     if (needsBroadcastA) {
  //       // 需要将2D的A广播为3D
  //       Value broadcastedA = performBroadcast2Dto3D(builder, originalA, batchDim, aType.getElementType());
  //       processedA = broadcastedA;
  //       LLVM_DEBUG(llvm::dbgs() << "Broadcasted A matrix for operation " << i << "\n");
  //     } else {
  //       // A已经是3D，直接使用
  //       processedA = originalA;
  //       LLVM_DEBUG(llvm::dbgs() << "Using original A matrix for operation " << i << "\n");
  //     }
      
  //     // 转换为memref并复制
  //     auto broadcastedAMemRefType = MemRefType::get(broadcastedAShape, aType.getElementType());
  //     Value aMemRef = builder.create<mlir::UnrealizedConversionCastOp>(
  //         builder.getUnknownLoc(), broadcastedAMemRefType, processedA).getResult(0);
  ...
  //     if (needsBroadcastB) {
  //       // 需要将2D的B广播为3D
  //       Value broadcastedB = performBroadcast2Dto3D(builder, originalB, batchDim, bType.getElementType());
  //       processedB = broadcastedB;
  //       LLVM_DEBUG(llvm::dbgs() << "Broadcasted B matrix for operation " << i << "\n");
  //     } else {
  //       // B已经是3D，直接使用
  //       processedB = originalB;
  //       LLVM_DEBUG(llvm::dbgs() << "Using original B matrix for operation " << i << "\n");
  //     }
  ...
  //         builder.getUnknownLoc(), broadcastedBMemRefType, processedB).getResult(0);
  ...
  //   LLVM_DEBUG(llvm::dbgs() << "All parallel input copies and broadcasts completed\n");
  ...
  //         gpu::AsyncTokenType::get(builder.getContext()),
  //         ValueRange(outputCopyStreams[i]),
  //         individualOutputs[i], outputView);
  //     outputCopyTokens.push_back(copyToken.getAsyncToken());
  //   }
  ...
  //   LLVM_DEBUG(llvm::dbgs() << "Successfully fused batch MatMul operations with broadcasting\n");
  //   return true;
  // }
