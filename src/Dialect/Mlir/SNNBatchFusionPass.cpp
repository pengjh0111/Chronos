...
//     LLVM_DEBUG(llvm::dbgs() << "Fusing " << group.operations.size() << " Conv operations\n");
...
//     // 分配GPU内存  
//     auto dep = builder.create<gpu::WaitOp>(builder.getUnknownLoc(), 
//         gpu::AsyncTokenType::get(builder.getContext()), ValueRange());
//     auto batchedInputAlloc = builder.create<gpu::AllocOp>(
//         builder.getUnknownLoc(), inputMemRefType, gpu::AsyncTokenType::get(builder.getContext()), 
//         ValueRange(dep.getAsyncToken()), ValueRange(), ValueRange());
//     builder.create<gpu::WaitOp>(builder.getUnknownLoc(), Type{}, ValueRange(batchedInputAlloc.getAsyncToken()));
//     Value batchedInput = batchedInputAlloc.getMemref();
    
//     auto dep1 = builder.create<gpu::WaitOp>(builder.getUnknownLoc(), 
//         gpu::AsyncTokenType::get(builder.getContext()), ValueRange());
//     auto batchedOutputAlloc = builder.create<gpu::AllocOp>(
//         builder.getUnknownLoc(), outputMemRefType, gpu::AsyncTokenType::get(builder.getContext()), 
//         ValueRange(dep1.getAsyncToken()), ValueRange(), ValueRange());
//     builder.create<gpu::WaitOp>(builder.getUnknownLoc(), Type{}, ValueRange(batchedOutputAlloc.getAsyncToken()));
//     Value batchedOutput = batchedOutputAlloc.getMemref();
...
//       auto dep = builder.create<gpu::WaitOp>(builder.getUnknownLoc(), 
//           gpu::AsyncTokenType::get(builder.getContext()), ValueRange());
//       auto individualOutputAlloc = builder.create<gpu::AllocOp>(
//           builder.getUnknownLoc(), originalOutputMemRefType, gpu::AsyncTokenType::get(builder.getContext()), 
//           ValueRange(dep.getAsyncToken()), ValueRange(), ValueRange());
//       builder.create<gpu::WaitOp>(builder.getUnknownLoc(), Type{}, ValueRange(individualOutputAlloc.getAsyncToken()));
//       individualOutputs.push_back(individualOutputAlloc.getMemref());
//     }
...
//           inputViews[i], inputMemRef);  // 目标，源
//       builder.create<gpu::WaitOp>(builder.getUnknownLoc(), Type{}, ValueRange(copyToken.getAsyncToken()));
      
//       LLVM_DEBUG(llvm::dbgs() << "Created input copy for operation " << i << "\n");
//     }
...
//     // ========== 关键修复：将融合结果复制到预分配的batched output memory ==========
...
//       LLVM_DEBUG(llvm::dbgs() << "Created output copy for operation " << i << "\n");
//     }
...
//     LLVM_DEBUG(llvm::dbgs() << "Successfully fused Conv operations\n");
//     return true;
//   }
  
//   // 融合MaxPool操作 - 修复版本
//   bool fuseMaxPoolOperations(OpBuilder& builder, FusibleGroup& group) {
//     LLVM_DEBUG(llvm::dbgs() << "Fusing " << group.operations.size() << " MaxPool operations\n");
...
//     LLVM_DEBUG(llvm::dbgs() << "Successfully fused MaxPool operations\n");
//     return true;
//   }
  
//   // 融合MatMul操作 - 修复版本
//   bool fuseMatMulOperations(OpBuilder& builder, FusibleGroup& group) {
//     LLVM_DEBUG(llvm::dbgs() << "Fusing " << group.operations.size() << " MatMul operations\n");
...
//     auto dep = builder.create<gpu::WaitOp>(builder.getUnknownLoc(), 
//         gpu::AsyncTokenType::get(builder.getContext()), ValueRange());
//     auto batchedAAlloc = builder.create<gpu::AllocOp>(
//         builder.getUnknownLoc(), aMemRefType, gpu::AsyncTokenType::get(builder.getContext()), 
//         ValueRange(dep.getAsyncToken()), ValueRange(), ValueRange());
//     builder.create<gpu::WaitOp>(builder.getUnknownLoc(), Type{}, ValueRange(batchedAAlloc.getAsyncToken()));
//     Value batchedA = batchedAAlloc.getMemref();
...
//     LLVM_DEBUG(llvm::dbgs() << "Successfully fused MatMul operations\n");
//     return true;
//   }

//   // 混合MatMul操作融合（包括Gemm(bias=0)和MatMul）- 修复版本
//   bool fuseMixedMatMulOperations(OpBuilder& builder, FusibleGroup& group) {
//     LLVM_DEBUG(llvm::dbgs() << "Fusing " << group.operations.size() << " mixed MatMul operations\n");
...
//     LLVM_DEBUG(llvm::dbgs() << "Successfully fused mixed MatMul operations\n");
//     return true;
//   }
...
private:
  ...
  bool fuseConvOperations(OpBuilder& builder, FusibleGroup& group) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "Allocated memory for batched input/output\n");
  ...
  // 混合MatMul操作融合（包括Gemm(bias=0)和MatMul）- 修改为使用memref.alloc
  bool fuseMixedMatMulOperations(OpBuilder& builder, FusibleGroup& group) {
    LLVM_DEBUG(llvm::dbgs() << "Fusing " << group.operations.size() << " mixed MatMul operations\n");
