...
//===----------------------------------------------------------------------===//
// 重构的并行循环Tiling Pass - 专注A100性能优化
//===----------------------------------------------------------------------===//

struct ParallelLoopTiling
    : public PassWrapper<ParallelLoopTiling, OperationPass<ModuleOp>> {
  
  // 构造函数接受noMinMaxBounds参数
  ParallelLoopTiling(bool noMinMaxBoundsParam = true) {
    noMinMaxBounds = noMinMaxBoundsParam;
  }
  ...
  StringRef getDescription() const final {
    return "Tile parallel loops with A100-optimized tile sizes using advanced performance modeling";
  }
  
  void runOnOperation() override;
  
  /// 使用优化配置对并行循环应用tiling变换
  std::pair<ParallelOp, ParallelOp> tileParallelLoopWithOptimizedConfig(
      ParallelOp op, 
      const CompleteTileConfig &tileConfig, 
      bool noMinMaxBounds = false);
};
...
void ParallelLoopTiling::runOnOperation() {
  ...
  LLVM_DEBUG(llvm::dbgs() << "Starting A100-optimized parallel loop tiling pass\n");
  
  // 创建A100优化器
  A100HardwareParameters hwParams; // 使用默认A100参数
  TileSizeOptimizer optimizer(context, hwParams);
  
  // 查找所有并行循环
  SmallVector<scf::ParallelOp, 8> parallelOps;
  module.walk([&](scf::ParallelOp op) {
    // 跳过有reduction的循环（暂时不支持）
  ...
  for (auto parallelOp : parallelOps) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "Processing parallel loop " << processedLoops 
                           << " with " << parallelOp.getNumLoops() << " dimensions\n");
    
    // 使用新的优化器获取最佳tile配置
    CompleteTileConfig tileConfig = optimizer.optimizeTileSize(parallelOp);
    
    // 检查是否找到了有效配置
    if (tileConfig.perDimConfig.empty()) {
      LLVM_DEBUG(llvm::dbgs() << "No valid tile configuration found, skipping\n");
      continue;
    }
    
    LLVM_DEBUG(llvm::dbgs() << "Found optimal configuration with score: " 
                           << tileConfig.overallPerformanceScore << "\n");
    LLVM_DEBUG(llvm::dbgs() << "Tile sizes: ");
    for (const auto &dimConfig : tileConfig.perDimConfig) {
      LLVM_DEBUG(llvm::dbgs() << dimConfig.tileSize << " ");
    }
    LLVM_DEBUG(llvm::dbgs() << "\n");
    LLVM_DEBUG(llvm::dbgs() << "Total threads per block: " << tileConfig.totalThreadsPerBlock << "\n");
    
    // 应用tiling变换
    auto tiledLoops = tileParallelLoopWithOptimizedConfig(parallelOp, tileConfig, noMinMaxBounds);
    
    if (tiledLoops.first && tiledLoops.second) {
      successfulOptimizations++;
      LLVM_DEBUG(llvm::dbgs() << "Successfully applied tiling\n");
    } else {
      LLVM_DEBUG(llvm::dbgs() << "Failed to apply tiling transformation\n");
      parallelOp->emitWarning() << "Failed to apply optimized tiling configuration";
    }
  }
  
  LLVM_DEBUG(llvm::dbgs() << "Tiling pass completed: " << successfulOptimizations 
                         << "/" << processedLoops << " loops successfully optimized\n");
}

std::pair<ParallelOp, ParallelOp> ParallelLoopTiling::tileParallelLoopWithOptimizedConfig(
    ParallelOp op, 
    const CompleteTileConfig &tileConfig,
    bool noMinMaxBounds) {
  ...
  // 验证配置
  if (tileConfig.perDimConfig.size() != op.getNumLoops()) {
    LLVM_DEBUG(llvm::dbgs() << "Configuration dimension mismatch: expected " 
                           << op.getNumLoops() << ", got " << tileConfig.perDimConfig.size() << "\n");
    return {nullptr, nullptr};
  }
  
  // 将tile大小转换为MLIR Values
  SmallVector<Value, 4> tileSizeConstants;
  tileSizeConstants.reserve(op.getUpperBound().size());
  
  for (size_t i = 0, end = op.getUpperBound().size(); i != end; ++i) {
    if (i < tileConfig.perDimConfig.size()) {
      auto tileSize = tileConfig.perDimConfig[i].tileSize;
      tileSizeConstants.push_back(b.create<arith::ConstantIndexOp>(loc, tileSize));
      
      LLVM_DEBUG(llvm::dbgs() << "Dimension " << i << " tile size: " << tileSize << "\n");
    } else {
      // 对于额外维度使用默认值1
      tileSizeConstants.push_back(b.create<arith::ConstantIndexOp>(loc, 1));
    }
  ...
  if (noMinMaxBounds && needInboundCheck) {
    ...
    // 构建边界检查条件
    Value inbound = b.create<arith::ConstantIntOp>(loc, 1, b.getIntegerType(1));
    ...
    auto ifInbound = b.create<scf::IfOp>(loc,
                                     /*resultTypes*/ llvm::ArrayRef<Type>{}, 
                                     inbound,
                                     /*hasElseRegion*/ false);
  ...
  LLVM_DEBUG(llvm::dbgs() << "Tiling transformation applied successfully\n");
