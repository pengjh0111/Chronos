...
struct StreamInfo {
  ...
  // 新增：判断这个stream是否需要优化(是否包含kernel或包装函数)
  bool shouldOptimize() const {
    return !wrapperCalls.empty() || !kernelLaunches.empty();
  }
};
...
public:
  ...
  void runOnOperation() override {
    ...
    if (failed(collectStreamInfo(funcOp, allStreamInfos))) {
      LLVM_DEBUG(llvm::dbgs() << "Failed to collect stream info\n");
      signalPassFailure();
      return;
    }
    
    LLVM_DEBUG(llvm::dbgs() << "Found " << allStreamInfos.size() 
               << " total stream sequences\n");
    
    // **关键修改：过滤出需要优化的stream**
    std::vector<StreamInfo> streamInfos;
    for (const auto& info : allStreamInfos) {
      if (info.shouldOptimize()) {
        streamInfos.push_back(info);
        LLVM_DEBUG(llvm::dbgs() << "  Stream with " << info.wrapperCalls.size() 
                   << " wrapper calls and " << info.kernelLaunches.size() 
                   << " kernel launches will be optimized\n");
      } else {
        LLVM_DEBUG(llvm::dbgs() << "  Skipping stream (no kernels or wrappers)\n");
      }
    }
    
    LLVM_DEBUG(llvm::dbgs() << "After filtering: " << streamInfos.size() 
               << " streams need optimization\n");
    
    // 如果只有0或1个需要优化的stream，无需优化
    if (streamInfos.size() <= 1) {
      LLVM_DEBUG(llvm::dbgs() << "Not enough streams to optimize\n");
      return;
    }
...
private:
  ...
  bool isWrapperFunction(StringRef funcName) {
    ...
    // cuBLAS/cuLibs 相关函数
    if (funcName.starts_with("mgpuCulibs")) {
      return true;
    }
    ...
    return funcName == "mgpuCulibsFullyConnectedForward" ||
           ...
           funcName == "mgpuCudnnMaxPoolForward" ||
           funcName == "mgpuCudnnConv2dForward" ||
           funcName == "mgpuCudnnBatchNormForward" ||
           funcName == "mgpuCudnnReLUForward" ||
           funcName == "mgpuCudnnSoftmaxForward";
  }
  
  // **新增：检查函数是否是内存操作(不需要优化的函数)**
  bool isMemoryOperation(StringRef funcName) {
    return funcName == "mgpuMemAlloc" ||
           funcName == "mgpuMemcpy" ||
           funcName == "mgpuMemcpyAsync" ||
           funcName == "mgpuMemFree" ||
           funcName == "mgpuMemset" ||
           funcName == "mgpuMemsetAsync";
  }
  ...
  LogicalResult collectStreamInfo(LLVM::LLVMFuncOp funcOp,
    ...
    funcOp.walk([&](Operation* op) {
      ...
      if (auto callOp = dyn_cast<LLVM::CallOp>(op)) {
        ...
        // **修改：跳过内存操作，只关注包装函数**
        else if (!isMemoryOperation(funcName) && isWrapperFunction(funcName)) {
          if (currentStream && callOp.getNumOperands() > 0) {
            // 最后一个参数是stream
            Value stream = callOp.getOperand(callOp.getNumOperands() - 1);
            if (streamMap.count(stream) && streamMap[stream] == currentStream) {
